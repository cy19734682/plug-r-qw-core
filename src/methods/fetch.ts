...
function handleRequest(method: string, url: string, data?: Collection, msg?: string, rPath?: string[],
  config?: RequestConfigR, isUrlData?: boolean) {
 ...
 * @returns {Promise<*>}
 */
function checkRequest(method: string, url: string, data: Collection = {}, msg?: string, rPath?: string[],
  config?: RequestConfigR, isUrlData?: boolean) {
  return new Promise((s, j) => {
    if (url) {
      if (config && config.spin) {
      ...
      if (window && window.hasOwnProperty('g')) {
        ...
        // @ts-ignore
        const _g = window.g
        let httpEnv = Object.keys(_g).filter(e => e.indexOf('URL') > -1).map(e => e.replace('URL', ''))
      ...
      if (config && config.headers && config.headers['Content-Type'] === 'multipart/form-data') {
        ...
        if (data && (!_.isEmpty(data))) {
          if (Array.isArray(data)) {
            ...
            data_ = {}
            for (let key in data) {
              if (data.hasOwnProperty(key) && (data[key] || data[key] === 0 || data[key] === false ||
...
export default {
   ...
   * @return {Promise<unknown>}
   */
  delete(url: string, data?: Collection, msg?: string, rPath?: string[], config?: RequestConfigR,
